...
def simulate_squad_mc(
    squad_posteriors: dict[
        int, dict
    ],  # dict: player_id -> {"position": str, "lambda_g":..., "lambda_a":..., "lambda_x":..., "lambda_gc":..., "lambda_yc":..., "lambda_rc":..., "alpha_0":..., "beta_0":..., "alpha_60":..., "beta_60":...}
    ...
    for player_id, post in squad_posteriors.items():  # Changed player_name to player_id
        ...
        lambda_x_samples = (
            post["lambda_x"][:N_sim]
            if post.get("lambda_x") is not None
            else np.zeros(N_sim)
        )
        lambda_gc_samples = (
            post["lambda_gc"][:N_sim]
            if post.get("lambda_gc") is not None
            else np.zeros(N_sim)
        )
        ...
        FDR_assists = (
            FDR_assists_dict[player_id] if FDR_assists_dict else FDR_goals
        )  # Changed player_name to player_id
        FDR_dc = (
            FDR_dc_dict[player_id] if FDR_dc_dict else np.ones(N_weeks)
        )  # Changed player_name to player_id
        FDR_cs = (
            FDR_cs_dict[player_id] if FDR_cs_dict else np.ones(N_weeks)
        )  # Changed player_name to player_id
