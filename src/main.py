...
def main() -> None:
    ...
    gk_idx_map, def_idx_map, mid_idx_map, fwd_idx_map = make_position_to_player_ids_list_map(player_position_map)
    gk_matches, def_matches, mid_matches, fwd_matches = filter_matches_by_position(matches_df)
    ...
    gk_posteriors = extract_posteriors(gk_trace, gk_idx_map, "Goalkeeper", minutes_params_gk, minutes_params_def, minutes_params_mid, minutes_params_fwd)
    def_posteriors = extract_posteriors(def_model, def_idx_map, "Defender", minutes_params_gk, minutes_params_def, minutes_params_mid, minutes_params_fwd)
    mid_posteriors = extract_posteriors(mid_model, mid_idx_map, "Midfielder", minutes_params_gk, minutes_params_def, minutes_params_mid, minutes_params_fwd)
    fwd_posteriors = extract_posteriors(fwd_model, fwd_idx_map, "Forward", minutes_params_gk, minutes_params_def, minutes_params_mid, minutes_params_fwd)
    ...
    FDR_goals_dict_players = {pid: np.array(fdr_dict[pid]['multipliers']) for pid in fdr_dict.keys()}
    FDR_assists_dict_players = {pid: np.array(fdr_dict[pid]['multipliers']) for pid in fdr_dict.keys()}
    FDR_dc_dict_players = {pid: np.array(fdr_dict[pid]['multipliers']) for pid in fdr_dict.keys()}
    FDR_cs_dict_players = {pid: np.array(fdr_dict[pid]['multipliers']) for pid in fdr_dict.keys()}
    ...
    # player_id -> name
    player_name_map = {row["player_id"]: f"{row['first_name']} {row['last_name']}"
                    for row in matches_df.select(["player_id", "first_name", "last_name"]).unique().to_dicts()}

    # player_id -> position
    player_position_map = {row["player_id"]: row["position"]
    ...
    mc_df.filter(
        pl.col('position') == 'Goalkeeper'
    ).sort('mean_points', descending=True).head(20)
